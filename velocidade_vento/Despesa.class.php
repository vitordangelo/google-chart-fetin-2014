	...
	class Despesas
	{
		public function pesquisaTodasAsDespesas()
		{
			...
			try {
				...
				$table = array();
				$table['cols'] = array(
										 array('label' =>'Hora da Leitura:',    'type' =>'string'),
										 array('label' => 'Velocidade do Vento', 'type' =>'number'),
				...
				$result = $conn -> query('SELECT * FROM historico where sensor_idsensor = 3'); //Realiza a consulta no BD.
				if ($result) 
				{	
					...
					{	
						...
						$id = $linha['hora']; //Guarda o valor do mes no objeto mes
